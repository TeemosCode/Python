...
def list_zip(city):
    ...
    if city == "台北市":
        print(zipcode[city].items())
    elif city == "新北市":
        print(zipcode[city].items())
    elif city == "基隆市":
        print(zipcode[city].items())
    else:
        print("查無資料，再次輸入要找的:(台北市，基隆市，新北市)")
        list_zip(input("輸入要查詢的城市名稱之郵遞區號(台北市，基隆市，新北市):"))
...
def area_to_zip(area):
    for x in zipcode:
        if zipcode[x].get(area , 0) != 0:
            print(zipcode[x][area])
